...
class PaymentCreateAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        amount = paid_course.price if paid_course else paid_lesson.price # Предполагается, что есть поле price в модели

        # Создание продукта в Stripe
        product_name = paid_course.name if paid_course else paid_lesson.name
        product_description = paid_course.description if paid_course else paid_lesson.description
        stripe_product_id = create_stripe_product(name=product_name, description=product_description)
        ...
        if not checkout_url:
            raise serializers.ValidationError("Could not create Stripe checkout session.")
...
class PaymentStatusAPIView(APIView):
    ...
    def get(self, request, session_id):
        ...
        try:
            ...
            return Response({'status': session.payment_status, 'payment_id': payment.id}, status=status.HTTP_200_OK)
